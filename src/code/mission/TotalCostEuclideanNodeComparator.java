...
public class TotalCostEuclideanNodeComparator extends NodeComparator {
	...
	// total cost is calculated by adding deaths in the path cost to deaths estimated by the euclidean heuristic,
	// and adding damage in the path cost to damage estimated by the euclidean heuristic
	public int[] getTotalCost(SearchTreeNode node) {
		String pathCost = node.getPathCost();
		int[] heuristicCost = new EuclideanHeuristicNodeComparator().getCostToNearestGoal(node);
