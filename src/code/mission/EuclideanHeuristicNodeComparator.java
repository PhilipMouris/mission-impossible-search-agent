...
public class EuclideanHeuristicNodeComparator extends NodeComparator {
	...
	// calculates the Euclidean distance between this member and ethan,
	...
	public int[] getCostToNearestGoal(SearchTreeNode node) {
		...
		for(int i = 0;i<remainingMembers.size();i++) {
			int distance = getEuclideanDistance(remainingMembers.get(i), ((MissionImpossibleState)node.getState()).getEthanPosition());
	...
	// Euclidean distance = Square Root((ethanRow - memberRow)^2 + (ethanColumn - memberColumn)^2)
	public int getEuclideanDistance(String member, String ethanPosition) {
		...
		return (int) Math.sqrt(Math.pow(ethanRow - memberRow, 2) + Math.pow(ethanColumn - memberColumn, 2));
	}
}
